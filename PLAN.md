...
Since the initial build, six rounds of hardening/redesign have landed:
...
- **Data integrity (2026-08-09)**: GDELT and Alpha Vantage had silently never produced a real reading in ~30 days of daily runs — every "success" was actually a demo-mode fallback, invisible because failures weren't logged and the `dataQuality` badge was a single global flag with no way to say which source was the problem. Root causes: the GDELT Doc API endpoint was failing daily in production (429s/timeouts) and, even when it worked, never carried the Goldstein scale or coordinates the app needs (those only exist in GDELT's Event table); Alpha Vantage's GitHub Actions secret held a stale key while the code itself was fine. Fixed by switching GDELT to its raw Event export mirror (with a country-code filter so domestic US noise doesn't drown out real international events) and rotating the Alpha Vantage secret; `fetchDailyQuote` now logs instead of failing silently. Also added unit tests for the scoring rubric and the `isGroundedInMetrics` grounding check (`src/lib/*.test.ts`, run via `npm test`), plus a CI workflow (`.github/workflows/test.yml`) so they run on every push/PR.
- **Per-lens data quality (2026-08-09)**: each lens (`LensReading.dataQuality`) now tracks quality from just the sources feeding it, rather than relying solely on the one digest-wide flag — a small "Last known reading"/"Demo data" badge shows next to the affected lens in both Skim and Read views (nothing shown when live, matching the existing banner's show-only-when-relevant convention). The financial-advice disclaimer was also fixed to render in Skim mode, not just Read — Skim is the default view, so it was previously invisible to most visitors.
- **Design pass (2026-08-09)**: fully realized the "Briefing Room" concept named below rather than just having the tokens for it — the five dials now mount on one continuous fixed-dark instrument panel (extending the existing "the dial has its own face regardless of theme" rule to the whole panel) instead of five isolated paper cards, with tick-mark detailing on the gauges. Also fixed real bugs this surfaced: the event map's stock OpenStreetMap tiles clashed with the palette (switched to CARTO's muted, theme-aware basemap, plus a severity legend that didn't exist before), the 7-day timeline had no cap on dots per day and broke once real GDELT volume arrived (capped at 6 + a "+N" chip), and the sector heatmap's warm/cool scale had no legend explaining it's a tension metaphor, not financial red/green convention.
- **Data expansion (2026-08-10)**: closed three gaps between the rubric below and the code — the 10yr Treasury yield and State Dept travel advisories were both named in the locked rubric from day one but never actually wired to a source (`scoreInvestments`/`scoreDailyRoutine` either lacked the parameter or always received `undefined`); both are now live (FRED `DGS10`; State Dept's official advisory RSS feed, filtered to a fixed major-hub-country allowlist since most Level 3/4 advisories are long-standing for low-traffic countries, not daily news). Also added metro-level Cost of Living for 12 major metros (real, verified BLS series — see `METRO_CPI_SERIES` in `src/lib/sources/bls.ts`), the first real personalization in the app: a "Prices You Pay, for:" picker persisted via localStorage, no accounts needed, swapping just that one lens while the other four stay national. Metro narratives use the deterministic template rather than a per-metro LLM call, keeping synthesis cost bounded to the five national lenses as originally intended.
