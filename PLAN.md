...
Phase 1 MVP is built and live: [smenon2710/geopolitical-forecast-monitor](https://github.com/smenon2710/geopolitical-forecast-monitor) on GitHub, deployed at [geopolitical-forecast-monitor.vercel.app](https://geopolitical-forecast-monitor.vercel.app). A GitHub Action (`.github/workflows/daily-refresh.yml`, 11:00 UTC + manual dispatch) regenerates the data daily and Vercel auto-deploys on every push; a second workflow (`.github/workflows/test.yml`) runs the unit tests and a typecheck on every push/PR. See [README.md](./README.md) for setup.

**Sources wired into lens scoring:** GDELT (raw Event export mirror, US-relevance filtered), FRED (CPI, 10yr Treasury `DGS10`, unemployment, average hourly earnings for real-wage growth, GDP), BLS (national CPI breakdown + 12-metro CPI), EIA (gasoline), USGS (significant quakes), NOAA (climate alerts), Alpha Vantage (indices + sector ETFs), US State Department (travel advisories, major-hub allowlist). FRED/EIA/NOAA/Alpha Vantage use API keys; GDELT/BLS/USGS/State Dept are keyless. LLM synthesis runs through OpenRouter. With no keys set, the pipeline runs entirely in mock mode (`MOCK_SOURCES` defaults to `true`).

Post-MVP hardening that has landed (see git history for the blow-by-blow):
- **Reliability** — per-source fetch timeouts with fallback to each source's last real cached reading rather than synthetic data (`src/lib/sources/fetchWithFallback.ts`, caches committed under `data/cache/`). Every fallback path now logs, after GDELT and Alpha Vantage were each found to have run in permanent demo mode since launch.
- **Plain language** — UI lens labels and the synthesis prompt both target a non-expert reader; raw technical labels (CPI, Goldstein scale, QoQ) stay available as cited detail.
- **Grounding** — every synthesis call is checked by `isGroundedInMetrics()`; ungrounded narratives are rejected for the fully-cited template fallback. Covered by unit tests (`src/lib/*.test.ts`).
- **Per-lens data quality** — `LensReading.dataQuality` tracks quality from only the sources feeding that lens; a "Last known reading"/"Demo data" badge shows per lens in both views when not live.
- **Visual design** — the "Briefing Room" instrument panel is fully realized (see "Visual Dashboard Layer" below): five SVG dial gauges on one continuous fixed-dark panel, CARTO theme-aware basemap with a severity legend, capped timeline dots, sector-heatmap legend.
- **Metro Cost of Living** — 12 major metros on verified BLS series (`METRO_CPI_SERIES` in `src/lib/sources/bls.ts`), picked via a localStorage-persisted selector with no accounts; swaps only the Cost of Living lens while the other four stay national. Metro narratives use the deterministic template, keeping LLM cost bounded to the five national lenses.
...
- **US map** (Leaflet with CARTO's muted, theme-aware basemap tiles — no key) plotting the day's flagged events by location with severity-coded markers and a matching severity legend.
- **Sector heatmap** (energy, tech, agriculture, defense, consumer goods) showing which sectors moved and by how much, on a diverging warm/cool scale with a legend noting it reads as tension, not the financial red/green convention.
...
- **Travel advisory "upgrade" detection** is currently a proxy (any major-hub country *currently at* Level 3/4), not a true day-over-day change — a real "upgrade" signal needs historical diffing against yesterday's feed, which this pass didn't build. Worth doing if false-"moderate" days on long-standing advisories turn out to be common.
- **Metro coverage is 12 fixed metros**, chosen for population/geographic spread and a verified BLS series, not user-searchable — expanding needs each new metro's series ID and publication cadence confirmed against the live API first (the BLS website itself blocks scraping; the API is separate and keyless).
